        ...
        <div className="container">
            <nav className="navbar ">
                <div className="container-fluid">
                    <div>
                        <Link to={"/"}>
                            <img src="https://www.freepnglogos.com/uploads/star-wars-logo-3.png" className="navbar-brand w-25 h-25" alt="Star Wars Logo" />
                        </Link>
                    </div>
                    <div className="dropdown-center">
                        ...
                        <ul className="dropdown-menu">
                            ...
                            {store.favList.length === 0 ? <li className="dropdown-item justify-content-between text-center text-secondary">(Add your favorites characters)</li> :
                                store.favList.map((ele, index) => (
                                    <div key={index} className="dropdown-item d-flex justify-content-between">
                                        ...
                                        <i className="fa-solid fa-trash-can d-flex justify-content-between m-2" onClick={() => actions.deleteFavChar(ele.name)}></i>
                                    </div>
                                ))}
                            ...
                            {store.vehiclesFavList.length === 0 ? <li className="dropdown-item text-center text-secondary">(Add your favorites vehicles)</li> :
                                store.vehiclesFavList.map((ele, index) => (
                                    <div key={index} className="dropdown-item d-flex justify-content-between">
                                        ...
                                        <i className="fa-solid fa-trash-can d-flex justify-content-between m-2" onClick={() => actions.deleteFavVehicle(ele.name)}></i>
                                    </div>
                                ))}
                            ...
                            {store.planetsFavList.length === 0 ? <li className="dropdown-item text-center text-secondary">(Add your favorites planets)</li> :
                                store.planetsFavList.map((ele, index) => (
                                    <div key={index} className="dropdown-item d-flex justify-content-between">
                                        ...
                                        <i className="fa-solid fa-trash-can d-flex justify-content-between m-2" onClick={() => actions.deleteFavPlanet(ele.name)}></i>
                                    </div>
                                ))}
                        </ul>
                    </div>
